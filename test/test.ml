let print_time time =
  Printf.printf "s = %s, ns = %s"
    (Int64.to_string Posix_time.Timespec.(time.tv_sec)) 
    (Int64.to_string Posix_time.Timespec.(time.tv_nsec)) 
...
let test name c =
  ...
  | Some clock ->
    begin
      print_string "\n * time: "; print (Posix_clock.clock_gettime clock);
      print_string "\n * resolution: "; print (Posix_clock.clock_getres clock); print_endline "";
    end
  | None -> print_endline "\n * unavailable"

let _ =
  let open Posix_clock.Clock in
  ...
  let () = match Posix_clock.clock_getcpuclockid 0 with
...
let _ =
  ...
  let sec1 = Posix_time.Timespec.create Int64.one Int64.zero in
  let () = match Posix_clock.nanosleep sec1 with
  ...
  let () = match Posix_clock.clock_nanosleep realtime sec1 with
    | Result.Ok t -> (begin match t with None -> print_string "without interrupt" | Some t -> print_time t end; print_endline "")
    | Result.Error (`EUnix err) -> begin print_string "error: "; print_endline (Unix.error_message err) end
  in
  print_endline "done"
